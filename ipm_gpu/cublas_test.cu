...
int main(int argc, char ** argv) {
    ...
    double *J_like_mat_1, *J_like_mat_2, *J_like_mat_3, *Q_like_mat_1;

    CHECK_CUDA(cudaMalloc((void **)&J_like_mat_1, n * n * 2 * sizeof(double)));
    CHECK_CUDA(cudaMalloc((void **)&J_like_mat_2, n * n * 2 * sizeof(double)));
    CHECK_CUDA(cudaMalloc((void **)&Q_like_mat_1, n * n * sizeof(double)));
    CHECK_CUDA(cudaMalloc((void **)&J_like_mat_3, n * n * 2 * sizeof(double)));

    cublasHandle_t handle; 
    CHECK_CUBLAS(cublasCreate(&handle), "Failed creating handle");

    matrix out_1(n, n);
    computation::gpu_mm_mult(J, J_like_mat_1, J, J_like_mat_2, out_1, Q_like_mat_1, true, false, handle);
    std::cout << "Success" << std::endl;

    matrix out_2(n, 2*n);
    computation::gpu_mm_mult(Q, Q_like_mat_1, J, J_like_mat_1, out_2, J_like_mat_3, true, false, handle);
    std::cout << "Success" << std::endl;
