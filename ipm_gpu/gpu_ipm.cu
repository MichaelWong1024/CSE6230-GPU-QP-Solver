...
int multi_ipm(matrix* Qs, int Q_num, vector* cs, matrix& J, vector& g, int itr) {
  cublasHandle_t handle; 
  CHECK_CUBLAS(cublasCreate(&handle), "Failed creating handle");
  ...
  double *J_like_mat_1, *J_like_mat_2, *Q_like_mat_1, *Q_like_mat_2;
  double *var_like_vec_1, *var_like_vec_2, *ineq_like_vec_1, *ineq_like_vec_2;
  CHECK_CUDA(cudaMalloc((void **)&J_like_mat_1, num_var * num_ineq * sizeof(double)));
  CHECK_CUDA(cudaMalloc((void **)&J_like_mat_2, num_var * num_ineq * sizeof(double)));
  CHECK_CUDA(cudaMalloc((void **)&Q_like_mat_1, num_var * num_var * sizeof(double)));
  CHECK_CUDA(cudaMalloc((void **)&Q_like_mat_2, num_var * num_var * sizeof(double)));
  CHECK_CUDA(cudaMalloc((void **)&var_like_vec_1, num_var * sizeof(double)));
  CHECK_CUDA(cudaMalloc((void **)&ineq_like_vec_1, num_ineq * sizeof(double)));
  CHECK_CUDA(cudaMalloc((void **)&var_like_vec_2, num_var * sizeof(double)));
  CHECK_CUDA(cudaMalloc((void **)&ineq_like_vec_2, num_ineq * sizeof(double)));
  ...
  for (int i=0; i<itr; i++) {
    ...
    for (int j=0; j<Q_num; j++) {
      ...
      computation::gpu_mv_mult(Q, Q_like_mat_1, z, var_like_vec_1, rd, var_like_vec_2, false, handle);          // rd = Q @ z + rd
      computation::gpu_mv_mult(J, J_like_mat_1, lambda, ineq_like_vec_1, rd_temp, var_like_vec_1, true, handle); // rd_temp  = J' @ lambda  + rd_temp
      ...
      computation::gpu_mm_mult(J, J_like_mat_1, LHS_4_temp, J_like_mat_2,
        LHS_4, Q_like_mat_1, true, false, handle);  // LHS_4 = J' @ LHS_4_temp + LHS_4
...
int main(int argc, char ** argv) {
    ...
    std::array<char, 128> buffer_2;
    std::string output_2;
  
    std::shared_ptr<FILE> pipe_2(popen(command.c_str(), "r"), pclose);
    if (!pipe_2) throw std::runtime_error("popen() failed!");

    while (fgets(buffer_2.data(), buffer_2.size(), pipe_2.get()) != nullptr) {
        output_2 += buffer_2.data();
    }

    // parse the output
    std::stringstream ss_2(output_2);
    std::string part_2;
    std::vector<double*> all_parts_2;
    std::vector<size_t> sizes_2;
    size_t size_2;

    while (getline(ss_2, part_2, ';')) {
        all_parts_2.push_back(parse_doubles_from_string(part_2, size_2));
        sizes_2.push_back(size_2);
    }

    double* Q_arr_2 = all_parts_2[0];
    std::cout << "Q size: " << sizes_2[0] << std::endl;
    matrix Q_2(n, n, Q_arr_2);
    
    double* c_arr_2 = all_parts_2[1];
    std::cout << "c size: " << sizes_2[1] << std::endl;
    vector c_2(n, c_arr_2);

    matrix Qs[2] = {Q, Q_2};
    vector cs[2] = {c, c_2};

    multi_ipm(Qs, 2, cs, J, g, 100);

    for (double* ptr : all_parts) {
        delete[] ptr;
    }
    for (double* ptr : all_parts_2) {
        delete[] ptr;
    }
}
