...
int ipm(matrix& Q, vector& c, matrix& J, vector& g, int itr) {
  ...
  for (int i=0; i<itr; i++) {
    ...
    computation::mv_mult(Q, z, rd, false);
    computation::mv_mult(J, lambda, rd_temp, true);
    computation::vv_add(rd_temp, rd, 1.0);
    computation::vv_add(c, rd, 1.0);
    // rd.display();

    // r_b = -J*z_k + g - t_k
    vector rb(num_ineq);
    vector rb_temp(num_ineq);
    computation::mv_mult(J, z, rb_temp, false);
    computation::vv_add(rb_temp, rb, -1.0);
    computation::vv_add(g, rb, 1.0);
    computation::vv_add(t, rb, -1.0);
    // rb.display();

    // LHS_4 = Q + J'(lambda_k * t_k * J)
    matrix LHS_4(num_var, num_var);
    matrix LHS_4_temp(J);
    computation::vm_mult(t, LHS_4_temp);
    computation::vm_mult(lambda, LHS_4_temp);
    computation::mm_mult(J, LHS_4_temp, LHS_4, true, false);
    computation::mm_add(Q, LHS_4, 1.0);
    // LHS_4.display();
    ...
    computation::v_inv(inv_t);
    computation::v_inv(inv_lambda);
    //inv_t.display();
    //inv_lambda.display();

    vector RHS_4_temp(inv_lambda);
    vector RHS_4(num_var);
    computation::sv_mult(STEP*mu, RHS_4_temp);
    computation::vv_add(t, RHS_4_temp, -1.0);
    computation::vv_add(rb, RHS_4_temp, -1.0);
    computation::vv_pw_mult(lambda, RHS_4_temp);
    computation::vv_pw_mult(inv_t, RHS_4_temp);
    computation::mv_mult(J, RHS_4_temp, RHS_4, true);
    computation::sv_mult(-1.0, RHS_4);
    computation::vv_add(rd, RHS_4, -1.0);
    //RHS_4.display();
    ...
    computation::sv_mult(STEP * mu, delta_L);
    computation::vv_add(lambda, delta_L, -1);
    vector delta_L_temp(num_ineq);
    computation::mv_mult(J, delta_z_l, delta_L_temp, false);
    computation::vv_add(rb, delta_L_temp, -1);
    computation::vv_pw_mult(inv_t, delta_L_temp);
    computation::vv_pw_mult(lambda, delta_L_temp);
    computation::vv_add(delta_L_temp, delta_L, 1.0);
    //std::cout << "Check" << std::endl;

    // delta_t = -t_k + inv(lambda_k) * (step * mu_k * e - T * delta_lambda);
    vector yee(num_ineq, posi_init);
    vector delta_t(delta_L);
    computation::vv_pw_mult(t, delta_t);
    computation::sv_mult(-1.0, delta_t);
    computation::vv_add(yee, delta_t, STEP * mu);
    computation::vv_pw_mult(inv_lambda, delta_t);
    computation::vv_add(t, delta_t, -1.0);
    //std::cout << "Check" << std::endl;
    ...
    computation::vv_add(RHS_4, z, alpha);
    computation::vv_add(delta_L, lambda, alpha);
    computation::vv_add(delta_t, t, alpha);
    //std::cout << "Check" << std::endl;
